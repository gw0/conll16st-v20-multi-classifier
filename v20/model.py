...
from tasks.rel_types import rel_types_model, encode_x_rel_types, decode_x_rel_types, rel_types_one_model, encode_x_rel_types_one, decode_x_rel_types_one
...
def build_model(max_len, embedding_dim, dropout_p, words2id_size, skipgram_offsets, pos_tags2id_size, rel_types2id_size, rel_senses2id_size, rel_marking2id_size):
    ...
    # model: discourse relation boundary markers (sample, time_pad, rel_marking2id)
    if 'x_rel_marking' in loss:
        rel_marking_out = rel_marking_model(model, ['shared_2'], max_len, embedding_dim, rel_marking2id_size)
        model.add_output(name='x_rel_marking', input=rel_marking_out)
...
def relation_sample(rel_id, token_start, token_end, max_len, doc_ids, words, word_metas, pos_tags, dependencies, parsetrees, rel_ids, rel_parts, rel_types, rel_senses, words2id, words2id_size, skipgram_offsets, pos_tags2id, pos_tags2id_size, rel_types2id, rel_types2id_size, rel_senses2id, rel_senses2id_size, rel_marking2id, rel_marking2id_size):
    ...
    x1_rel_marking = encode_x1_rel_marking(word_metas_slice, rel_marking2id, rel_marking2id_size, max_len, filter_prefixes=[rel_tag])

    x1_rel_types_one = encode_x_rel_types_one(rel_types[rel_id], rel_types2id, rel_types2id_size)
    x1_rel_senses_one = encode_x_rel_senses_one(rel_senses[rel_id], rel_senses2id, rel_senses2id_size)
    return x1_words_pad, x1_words_rand, x1_skipgram, x1_pos_tags, x1_rel_types, x1_rel_senses, x1_rel_focus, x1_rel_marking, x1_rel_types_one, x1_rel_senses_one
...
class RelationMetrics(Callback):
    """Callback for other discourse relation metrics on validation data."""

    def __init__(self, prefix, data, batch_size, word_crop, max_len, doc_ids, words, word_metas, pos_tags, dependencies, parsetrees, rel_ids, rel_parts, rel_types, rel_senses, relations_gold, words2id, words2id_size, skipgram_offsets, pos_tags2id, pos_tags2id_size, rel_types2id, rel_types2id_size, rel_senses2id, rel_senses2id_size, rel_marking2id, rel_marking2id_size):
        super(RelationMetrics, self).__init__()
        self.prefix = prefix
        self.data = data
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        for i in range(len(x['_rel_id'])):
            ...
            if 'x_rel_types_one' in y:
                rel_type_one, rel_type_one_totals = decode_x_rel_types_one(y['x_rel_types_one'][i], self.rel_types2id, self.rel_types2id_size)
                if rel_type_one == self.rel_types[rel_id]:
                    rel_types_one_matching += 1
